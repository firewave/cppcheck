...
@pytest.mark.skipif(not has_clang_tidy, reason='clang-tidy is not available')
def test_clang_tidy_error_exit(tmp_path):
    test_file = tmp_path / 'test.cpp'
    with open(test_file, 'wt') as f:
        f.write(
"""#include <string>
...
static bool f() // NOLINT(misc-use-anonymous-namespace)
{
    ...
    # TODO: remove project file usage when --clang-tidy with non-project files
    ...
    assert stdout.splitlines() == []
    assert stderr.splitlines() == [
           "{}:10:12: style: 'str' used after it was moved [clang-tidy-bugprone-use-after-move,hicpp-invalid-access-moved]".format(test_file)
    ]
    assert exitcode == 12, stdout
