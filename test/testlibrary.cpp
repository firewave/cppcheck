...
private:
    ...
    void function_match_args() const {
        ...
        const std::string code("foo();"); // <- too few arguments, not library function
    ...
    void function_match_args_default() const {
        ...
        {
            TokenList tokenList(&settings);
            const std::string code("foo(a);"); // <- library function
            ASSERT(tokenList.createTokens(code, Standards::Language::CPP));
            Token::createMutualLinks(tokenList.front()->next(), tokenList.back()->previous());
            tokenList.createAst();
        ...
        {
            TokenList tokenList(&settings);
            const std::string code("foo(a, b);"); // <- library function
            ASSERT(tokenList.createTokens(code, Standards::Language::CPP));
            Token::createMutualLinks(tokenList.front()->next(), tokenList.back()->previous());
            tokenList.createAst();
        ...
        {
            TokenList tokenList(&settings);
            const std::string code("foo(a, b, c);"); // <- too much arguments, not library function
            ASSERT(tokenList.createTokens(code, Standards::Language::CPP));
            Token::createMutualLinks(tokenList.front()->next(), tokenList.back()->previous());
            tokenList.createAst();
