...
#define tok(...) tok_(__FILE__, __LINE__, __VA_ARGS__)
    ...
    std::string tok_(const char* file, int line, const char (&code)[size], Platform::Type type = Platform::Type::Native, bool debugwarnings = true, bool preprocess = false) {
        ...
        if (preprocess) {
            ...
            FileWithDetails f("test.cpp", Standards::Language::CPP, size-1);
            ASSERT_LOC(tokenizer.list.createTokens(istr, f), file, line); // TODO: this creates the tokens a second time
