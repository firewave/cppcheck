...
private:
    const Settings settings0 = settingsBuilder().severity(Severity::style).certainty(Certainty::inconclusive).build();
...
#define tok(...) tok_(__FILE__, __LINE__, __VA_ARGS__)
    std::string tok_(const char* file, int line, const char code[], bool simplify = true, bool debugwarnings = true) {
        errout.str("");

        // show warnings about unhandled typedef
        const Settings settings = settingsBuilder(settings0).debugwarnings(debugwarnings).platform(cppcheck::Platform::Type::Native).build();
...
#define checkSimplifyTypedef(code) checkSimplifyTypedef_(code, __FILE__, __LINE__)
    void checkSimplifyTypedef_(const char code[], const char* file, int line) {
        ...
        // Tokenize..
        // show warnings about unhandled typedef
        const Settings settings = settingsBuilder(settings0).debugwarnings().build();
        Tokenizer tokenizer(&settings, this);
        std::istringstream istr(code);
        ASSERT_LOC(tokenizer.tokenize(istr, "test.cpp"), file, line);
    }
    ...
    void simplifyTypedef14() {
        ...
        // Tokenize and check output..
        TODO_ASSERT_THROW(tok(code, true, false), InternalError); // TODO: Do not throw exception
        //ASSERT_EQUALS("", errout.str());
    }
    ...
    void simplifyTypedef62() { // ticket #2082
        ...
        ASSERT_EQUALS(expected2, tok(code2, false, false));
        ASSERT_EQUALS("", errout.str());
        ...
        ASSERT_EQUALS(expected3, tok(code3, false, false));
        ASSERT_EQUALS("", errout.str());
        ...
        ASSERT_EQUALS(expected4, tok(code4, false, false));
        ASSERT_EQUALS("", errout.str());
    }
    ...
    void simplifyTypedef65() { // ticket #2314
        const char code[] = "typedef BAR<int> Foo;\n"
                            ...
                            "    return b > Foo(10);\n"
                            "}";
        const std::string actual(tok(code, true, false));
    ...
    void simplifyTypedef75() { // ticket #2426
        ...
        ASSERT_EQUALS(";", tok(code, true, false));
        ASSERT_EQUALS("", errout.str());
    }
    ...
    void simplifyTypedef95() { // ticket #2844
        ...
        const char expected[] = "class symbol_table { "
                                ...
                                "} ;";
        ASSERT_EQUALS(expected, tok(code, true, false));
        ASSERT_EQUALS("", errout.str());
    }
