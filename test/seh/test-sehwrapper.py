...
def test_assert():
    ...
    # Assertion failed: false, file S:\GitHub\cppcheck-fw\test\seh\test-sehwrapper.cpp, line 35
    assert stderr.startswith("Assertion failed: false, file "), stderr
    assert stderr.endswith("test-sehwrapper.cpp, line 35\n"), stderr
    assert exitcode == 3
...
def test_segv():
    ...
    assert lines[0] == 'Internal error: Access violation (instruction: 0x00007FF7B54210E2) reading from 0x0000000000000000'
    assert lines[1].startswith('0. 0x')
    assert lines[1].endswith(' in my_segv')
    assert lines[1].startswith('1. 0x')
    assert lines[1].endswith(' in main')
