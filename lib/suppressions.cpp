...
bool Suppressions::Suppression::isSuppressed(const ErrorMessage &errmsg) const
{
    ...
    if (type == Type::macro) {
        ...
        if (!fileName.empty() && !matchglob(fileName, errmsg.getFileName()))
            return false;
        if ((Type::unique == type) && (lineNumber != NO_LINE) && (lineNumber != errmsg.lineNumber)) {
            if (!thisAndNextLine || lineNumber + 1 != errmsg.lineNumber)
                return false;
        }
        if ((Type::block == type) && ((errmsg.lineNumber < lineBegin) || (errmsg.lineNumber > lineEnd)))
            return false;
    }
    if (!symbolName.empty()) {
...
bool Suppressions::isSuppressed(const ::ErrorMessage &errmsg, const std::set<std::string>& macroNames)
{
    if (mSuppressions.empty())
        return false;
    return isSuppressed(ErrorMessage::fromErrorMessage(errmsg, macroNames));
}
    ...
    for (const Token *tok = tokenizer.tokens(); tok; tok = tok->next()) {
        if (currFileIdx != tok->fileIndex() || currLineNr != tok->linenr()) {
            ...
            for (auto &suppression : mSuppressions) {
                if (suppression.type == Type::unique) {
                ...
                } else if (suppression.type == Type::block) {
