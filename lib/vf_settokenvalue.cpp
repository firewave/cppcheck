...
namespace ValueFlow
{
    ...
    static void setTokenValueCast(Token *parent, const ValueType &valueType, const Value &value, const Settings &settings)
    {
        if (valueType.pointer || value.isImpossible())
            ...
            if (value.isIntValue())
                floatValue.floatValue = (double)value.intvalue;
        ...
        // Calculations..
            ...
            for (const Value &value1 : parent->astOperand1()->values()) {
                ...
                for (const Value &value2 : parent->astOperand2()->values()) {
                    ...
                    const double floatValue1 = value1.isFloatValue() ? value1.floatValue : (double)value1.intvalue;
                    const double floatValue2 = value2.isFloatValue() ? value2.floatValue : (double)value2.intvalue;
