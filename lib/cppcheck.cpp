...
{
    return checkString(Path::simplifyPath(path), emptyString, content);
}
...
unsigned int CppCheck::checkString(const std::string& filename, const std::string &cfgname, const std::string& str)
{
    return checkInternal(filename, cfgname,
        [&filename, &str](TokenList& list) {
            list.createTokens(str, filename);
        },
        [&filename, &str](std::vector<std::string>& files, simplecpp::OutputList* outputList) {
            return simplecpp::TokenList{str, files, filename, outputList};
        });
}
