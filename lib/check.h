...
protected:
    ...
    /** report an error */
    void reportError(const Token *tok, const Severity::Type severity, const std::string &id, const std::string &msg) {
        reportError(tok, severity, id, msg, CWE(0U), Certainty::normal);
    }

    /** report an error */
    void reportError(const Token *tok, const Severity::Type severity, const std::string &id, const std::string &msg, const CWE &cwe, Certainty certainty) {
        const std::list<const Token *> callstack(1, tok);
        reportError(callstack, severity, id, msg, cwe, certainty);
    }

    /** report an error */
    void reportError(const std::list<const Token *> &callstack, Severity::Type severity, const std::string &id, const std::string &msg) {
        reportError(callstack, severity, id, msg, CWE(0U), Certainty::normal);
    }

    /** report an error */
    void reportError(const std::list<const Token *> &callstack, Severity::Type severity, const std::string &id, const std::string &msg, const CWE &cwe, Certainty certainty);

    void reportError(const ErrorPath &errorPath, Severity::Type severity, const char id[], const std::string &msg, const CWE &cwe, Certainty certainty);
