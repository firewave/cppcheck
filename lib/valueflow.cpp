                                             ...
                                             const Settings& settings)
{
    ...
    if (const Token* initList = constructor->constructorMemberInitialization()) {
        std::unordered_map<const Variable*, ValueFlow::LifetimeCapture> paramCapture;
        for (const Token* tok2 : astFlatten(initList->astOperand2(), ",")) {
            ...
            if (var->isReference() || var->isRValueReference()) {
                if (argvar && argvar->isArgument() && (argvar->isReference() || argvar->isRValueReference())) {
                    paramCapture[argvar] = ValueFlow::LifetimeCapture::ByReference;
                }
            } else {
                bool found = false;
                for (const ValueFlow::Value& v : expr->values()) {
                    ...
                    if (!lifeVar)
                        continue;
                    ValueFlow::LifetimeCapture c = ValueFlow::LifetimeCapture::Undefined;
                    if (!v.isArgumentLifetimeValue() && (lifeVar->isReference() || lifeVar->isRValueReference()))
                        c = ValueFlow::LifetimeCapture::ByReference;
                    else if (v.isArgumentLifetimeValue())
                        c = ValueFlow::LifetimeCapture::ByValue;
                    if (c != ValueFlow::LifetimeCapture::Undefined) {
                        paramCapture[lifeVar] = c;
                        found = true;
                    }
                }
                if (!found && argvar && argvar->isArgument())
                    paramCapture[argvar] = ValueFlow::LifetimeCapture::ByValue;
            }
        }
        ...
        LifetimeStore::forEach(tokenlist,
            ...
            if (paramCapture.count(paramVar) == 0)
                return;
            const ValueFlow::LifetimeCapture c = paramCapture.at(paramVar);
            if (c == ValueFlow::LifetimeCapture::ByReference)
    ...
    for (Token *tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        if (tok->variable() && Token::Match(tok, "%var% (|{|;")) {
            ...
            if (Token::simpleMatch(tok->astParent(), ". reset (")) {
                ...
                ValueFlow::Value v = ValueFlow::makeSymbolic(tok);
                          ...
                          TimerResultsIntf* timerResults)
{
    ...
    runner.run_once({
        ...
        VFA(analyzeLifetime(tokenlist, errorLogger, settings)),
    ...
    runner.run({
        ...
        VFA(analyzeSymbolicOperators(symboldatabase, settings)),
