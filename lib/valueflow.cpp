...
Analyzer::Result ValueFlow::valueFlowForward(Token* startToken,
                                         const Token* endToken,
                                         const Token* exprTok,
...
Analyzer::Result ValueFlow::valueFlowForwardRecursive(Token* top,
                                                  const Token* exprTok,
                                                  std::list<ValueFlow::Value> values,
                                                  const TokenList& tokenlist,
                                                  ErrorLogger& errorLogger,
...
void ValueFlow::valueFlowReverse(Token* tok,
...
static void valueFlowReverse(const TokenList& tokenlist,
                             ...
                             SourceLocation loc = SourceLocation::current())
{
    ValueFlow::valueFlowReverse(tok, nullptr, varToken, {std::move(val)}, tokenlist, errorLogger, settings, loc);
}
    ...
    for (const Scope * scope : symboldatabase.functionScopes) {
        ...
        for (auto* tok = const_cast<Token*>(scope->bodyStart); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (isEscapeScope(blockTok, settings)) {
                ...
                if (ValueFlow::isBreakOrContinueScope(blockTok->link())) {
                    scope2 = ValueFlow::getLoopScope(blockTok->link());
                    if (!scope2)
                        continue;
                }
                for (const Token* condTok2:conds) {
