...
static bool isNotEqual(std::pair<const Token*, const Token*> x, const std::string& y, bool cpp)
{
    ...
    tokenList.createTokens(y, cpp ? Standards::Language::CPP : Standards::Language::C);
...
static void valueFlowConditionExpressions(const TokenList &tokenlist, const SymbolDatabase& symboldatabase, ErrorLogger *errorLogger, const Settings &settings)
{
    if (!settings.daca && (settings.checkLevel == Settings::CheckLevel::normal))
    {
        if (errorLogger) {
            ...
            const ErrorMessage errmsg({std::move(loc)}, tokenlist.getSourceFilePath(), Severity::information, "Analysis of condition expressions is disabled. Use --check-level=exhaustive to enable it.", "normalCheckLevelConditionExpressions", Certainty::normal);
            errorLogger->reportErr(errmsg);
        }
        return;
    }
...
static bool getMinMaxValues(const std::string &typestr, const Settings &settings, bool cpp, MathLib::bigint &minvalue, MathLib::bigint &maxvalue)
{
    ...
    const std::string code(typestr+";");
    if (!typeTokens.createTokens(code, cpp ? Standards::Language::CPP : Standards::Language::C))
        return false;
