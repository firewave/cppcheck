    ...
    mNameToken(nullptr), mParamEnd(nullptr), mFlags(0)
{
    if (mToken) {
        if (mToken->strAt(1) == "<") {
            const Token * const end = mToken->next()->findClosingBracket();
            if (end && end->strAt(1) == "(") {
                isFunction(true);
            }
        }
    ...
    mNameToken(nameToken), mParamEnd(paramEnd), mFlags(0)
{
    ...
    if (mToken && mNameToken && mParamEnd) {
        ...
        if (tok1->str() == "<") {
            const Token * const closing = tok1->findClosingBracket();
            if (closing)
                tok1 = closing->next();
            else
...
{
    const Token * const end = aliasEndToken();
...
void TemplateSimplifier::checkComplicatedSyntaxErrorsInTemplates()
{
    ...
    for (const Token *tok = mTokenList.front(); tok; tok = tok->next()) {
        ...
        const Token * const start = Tokenizer::startOfExecutableScope(tok);
        if (start) {
            tok = start->link();
        }
...
unsigned int TemplateSimplifier::templateParameters(const Token *tok)
{
    ...
    while (tok) {
        ...
        if (level == 0 && Token::simpleMatch(tok, "template <")) {
            const Token * const closing = tok->next()->findClosingBracket();
            if (closing) {
                if (closing->str() == ">>")
...
bool TemplateSimplifier::removeTemplate(Token *tok)
{
    ...
    Token * const end = findTemplateDeclarationEnd(tok);
    if (end && end->next()) {
...
bool TemplateSimplifier::getTemplateDeclarations()
{
    ...
    for (Token *tok = mTokenList.front(); tok; tok = tok->next()) {
        ...
        while (tok1 && tok1->next()) {
            const Token * const closing = tok1->next()->findClosingBracket();
        ...
        for (const Token *tok2 = parmEnd; tok2; tok2 = tok2->next()) {
            ...
            // skip decltype(...)
                ...
                if (namepos > 0) {
                    TokenAndName decl(tok, tok->scopeInfo()->name, parmEnd->tokAt(namepos), parmEnd); // FP
...
void TemplateSimplifier::addInstantiation(Token *token, const std::string &scope)
{
    ...
    const TokenAndName instantiation(token, scope);
...
static void getFunctionArguments(const Token *nameToken, std::vector<const Token *> &args)
{
    ...
    if (nameToken->strAt(1) == "(")
        ...
        const Token * const end = nameToken->next()->findClosingBracket();
        if (end)
    ...
    for (Token *tok = mTokenList.front(); tok; tok = tok->next()) {
        ...
        if (Token::simpleMatch(tok, "template <")) {
            ...
            if (isUsing && Token::Match(tok->tokAt(2), "%name% <")) {
                ...
                Token * const tok2 = Token::findsimplematch(tok->tokAt(3), ";");
                if (tok2)
                    tok = tok2;
            } else if (tok->strAt(-1) == "<") {
                ...
                Token * const tok2 = findTemplateDeclarationEnd(tok->next());
                if (tok2)
                    tok = tok2;
            }
            ...
            // Can't have specialized type alias so ignore it
            Token * const tok2 = Token::findsimplematch(tok->tokAt(2), ";");
            if (tok2)
                tok = tok2;
                ...
                for (auto pos = range.first; pos != range.second; ++pos) {
                    ...
                    if (pos->second->fullName() == fullName ||
                        ...
                        if (match == declarationParams.size()) {
                            const Token * const arg = instantiationArgs[argMatch];
                            tok->insertToken(">");
                            switch (arg->tokType()) {
                            case Token::eBoolean:
                            ...
                            case Token::eString:
                                ...
                                break;
                            case Token::eNumber: {
                                const MathLib::value num(arg->str());
                                if (num.isFloat()) {
                                    ...
                                    const std::string suffix = MathLib::getSuffix(tok->strAt(3));
                ...
                while (true) {
                    ...
                    if (it != mTemplateDeclarations.end()) {
                        ...
                        for (const auto & nameSpace :  tok->scopeInfo()->usingNamespaces) {
                            const std::string fullNameSpace = scopeName + (scopeName.empty()?"":" :: ") +
                                                        nameSpace + (qualification.empty()?"":" :: ") + qualification;
                            const std::string newFullName = fullNameSpace + " :: " + tok->str();
...
void TemplateSimplifier::useDefaultArgumentValues(TokenAndName &declaration)
{
    ...
    for (Token *tok = declaration.token()->next(); tok; tok = tok->next()) {
        if (Token::simpleMatch(tok, "template <")) {
            Token * const end = tok->next()->findClosingBracket();
            if (end)
                tok = end;
            continue;
        }
    ...
    for (const TokenAndName &instantiation : mTemplateInstantiations) {
        ...
        std::size_t index = 0;
        const Token * const end = instantiation.token()->next()->findClosingBracket();
        if (!end)
            continue;
        ...
        for (const Token *tok1 = instantiation.token()->tokAt(2); tok1 && tok1 != end; tok1 = tok1->next()) {
            if (tok1->link() && Token::Match(tok1, "{|(|[")) {
                const Token * const endLink = tok1->link();
                do {
                ...
                const Token * const endLink = tok1->findClosingBracket();
                do {
                    instantiationArgs[index].push_back(tok1);
                    tok1 = tok1->next();
                } while (tok1 && tok1 != endLink);
        ...
        Token * const instantiationEnd = tok->findClosingBracket();
            ...
            while (it != eq.end()) {
                ...
                for (const Token* from = it->eq->next(); from && from != it->end; from = from->next()) {
                    ...
                    if (entry != typeParameterNames.end() && entry->second < instantiationArgs.size()) {
                        for (const Token * const tok1 : instantiationArgs[entry->second]) {
...
void TemplateSimplifier::simplifyTemplateAliases()
{
    for (std::list<TokenAndName>::iterator it1 = mTemplateDeclarations.begin(); it1 != mTemplateDeclarations.end();) {
        ...
        for (std::list<TokenAndName>::iterator it2 = mTemplateInstantiations.begin(); it2 != mTemplateInstantiations.end();) {
            ...
            Token * const dst = aliasUsage.token()->next()->findClosingBracket();
            Token * const end = TokenList::copyTokens(dst, aliasDeclaration.aliasStartToken(), aliasDeclaration.aliasEndToken()->previous(), false)->next();
            ...
            for (Token *tok1 = dst->next(); tok1 != end; tok1 = tok1->next()) {
                ...
                if (aliasParameterNames.find(tok1->str()) != aliasParameterNames.end()) {
                    ...
                    Token * const temp = TokenList::copyTokens(tok1, fromStart, fromEnd, true);
        ...
        if (found) {
            Token * const end = const_cast<Token *>(aliasDeclaration.aliasEndToken());
...
bool TemplateSimplifier::instantiateMatch(const Token *instance, const std::size_t numberOfArguments, bool variadic, const char patternAfter[])
{
    ...
    if (patternAfter) {
        const Token * const tok = instance->next()->findClosingBracket();
...
bool TemplateSimplifier::getTemplateNamePositionTemplateFunction(const Token *tok, int &namepos)
{
    ...
    while (tok && tok->next()) {
        ...
        // skip decltype(...)
            ...
            const Token * const end = tok->linkAt(2)->previous();
            while (tok->next() && tok != end) {
                tok = tok->next();
                namepos++;
            }
        } else if (Token::Match(tok->next(), "%type% <")) {
            const Token * const closing = tok->tokAt(2)->findClosingBracket();
            if (closing) {
...
bool TemplateSimplifier::getTemplateNamePositionTemplateVariable(const Token *tok, int &namepos)
{
    ...
    while (tok && tok->next()) {
        ...
        // skip decltype(...)
            ...
            const Token * const end = tok->linkAt(2);
            while (tok->next() && tok != end) {
                tok = tok->next();
                namepos++;
            }
...
void TemplateSimplifier::addNamespace(const TokenAndName &templateDeclaration, const Token *tok)
{
    ...
    while ((end = templateDeclaration.scope().find(" ", start)) != std::string::npos) {
        const std::string token = templateDeclaration.scope().substr(start, end - start);
    ...
    bool copy)
{
    ...
    if (copy && isClass) {
        ...
        if (!isSpecialization && it != mTemplateForwardDeclarationsMap.end()) {
            ...
            const Token * const temp1 = dst->tokAt(1)->findClosingBracket();
            const Token * const temp2 = temp1->tokAt(getTemplateNamePosition(temp1));
            start = temp1->next();
            ...
            if (itype < typeParametersInDeclaration.size() && itype < mTypesUsedInTemplateInstantiation.size() &&
                ...
                if (isSpecialization && !copy && !scope.empty() && Token::Match(start, (scope + templateDeclarationNameToken->str()).c_str())) {
                    ...
                    if (start->strAt(1) == "<") {
                        ...
                        Token * const closing = start->next()->findClosingBracket();
                        if (closing) {
                            std::string name;
                ...
                if (start->link()) {
                    if (Token::Match(start, "[|{|(")) {
                        ...
                        const std::map<const Token *, Token *>::iterator link = links.find(start);
            ...
            while (tok5 && tok5 != tok3) {
                ...
                if (Token::Match(tok5, "%name% <") && tok5->str() == templateInstantiation.name()) {
                    ...
                    if (!added) {
                        ...
                        Token * const back = mTokenList.back();
                        if (Token::Match(back, "{|(|[")) {
                            brackets2.push(back);
                        } else if (back->str() == "}") {
        ...
        for (; tok3; tok3 = tok3->next()) {
            ...
            if (tok3->str() == lastName) {
                if (Token::simpleMatch(tok3->next(), "<")) {
                    Token * const closingBracket = tok3->next()->findClosingBracket();
                    if (closingBracket) {
            ...
            if (Token::simpleMatch(tok3, "template <")) {
                Token * const tok2 = findTemplateDeclarationEnd(tok3);
                if (tok2)
                    templates.push(tok2);
            ...
            if (Token::Match(tok3, "%type% <") &&
                ...
                const Token * const closingBracket = tok3->next()->findClosingBracket();
            ...
            // link() newly tokens manually
                ...
                if (tok3->str() == "{") {
                    ...
                    if (tok3->strAt(1) == ";") {
                        const Token * const tokSemicolon = tok3->next();
...
bool TemplateSimplifier::simplifyNumericCalculations(Token *tok, bool isTemplate)
{
    ...
    while (tok->tokAt(3) && tok->isNumber() && tok->tokAt(2)->isNumber()) { // %any% %num% %any% %num% %any%
        const Token * const before = tok->previous();
        if (!before)
            break;
        const Token * const op = tok->next();
        const Token * const after = tok->tokAt(3);
...
void TemplateSimplifier::simplifyTemplateArgs(Token *start, Token *end)
{
    ...
    Token * const first = (start && start->previous()) ? start->previous() : mTokenList.front();
    bool again = true;
    ...
    std::vector<const Token *> & typeParametersInDeclaration)
{
    ...
    const Token * const end = tok->previous()->findClosingBracket();
    ...
    for (; tok && tok!= end; tok = tok->next()) {
        if (Token::simpleMatch(tok, "template <")) {
            ...
            if (tok->str() == ",")
                ...
                const Token * const closing = tok->findClosingBracket();
                if (closing)
                    tok = closing;
            }
        }
    ...
    std::list<std::string> &typeStringsUsedInTemplateInstantiation)
{
    ...
    const Token * const endToken = tok2->next()->findClosingBracket();
    ...
    std::set<std::string> &expandedtemplates)
{
    ...
    for (const TokenAndName &instantiation : mTemplateInstantiations) {
        ...
        if (templateDeclaration.isFunction() && instantiation.isFunction()) {
            ...
            if (declFuncArgs.size() != instFuncParams.size()) {
                ...
                const Token * const end = templateDeclaration.nameToken()->linkAt(1);
                size_t count = 0;
        ...
        while (Token::Match(startToken->tokAt(-2), ">|%name% :: %name%")) {
            if (startToken->strAt(-2) == ">") {
                const Token * const tok3 = startToken->tokAt(-2)->findOpeningBracket();
                if (tok3)
        ...
        std::list<std::string> typeStringsUsedInTemplateInstantiation;
        const std::string typeForNewName = getNewName(tok2, typeStringsUsedInTemplateInstantiation);
    ...
    if (!instantiated && specialized) {
        Token * const tok2 = const_cast<Token *>(templateDeclaration.nameToken());
        if (mErrorLogger && !mTokenList.getFiles().empty())
            mErrorLogger->reportProgress(mTokenList.getFiles()[0], "TemplateSimplifier::simplifyTemplateInstantiations()", tok2->progressValue());
#ifdef MAXTIME
        if (std::time(0) > maxtime)
...
static bool matchTemplateParameters(const Token *nameTok, const std::list<std::string> &strings)
{
    ...
    const Token * const end = nameTok->next()->findClosingBracket();
    if (!end)
        return false;
    ...
    const std::string &newName)
{
    ...
    for (Token *nameTok = mTokenList.front(); nameTok; nameTok = nameTok->next()) {
        ...
        std::set<TemplateSimplifier::TokenAndName*> * const pointers = nameTok->templateSimplifierPointers();
        ...
        Token * const tok2 = nameTok->next()->findClosingBracket();
...
void TemplateSimplifier::printOut(const TokenAndName &tokenAndName, const std::string &indent) const
{
    ...
    if (tokenAndName.token() && !tokenAndName.paramEnd() && tokenAndName.token()->strAt(1) == "<") {
        const Token * const end = tokenAndName.token()->next()->findClosingBracket();
        if (end) {
    ...
    bool &codeWithTemplates)
{
    ...
    if (mSettings->standards.cpp >= Standards::CPP20) {
        ...
        for (Token *tok = mTokenList.front(); tok; tok = tok->next()) {
            if (!Token::Match(tok, ")|>|>> requires %name%|("))
                continue;
            Token * const end = skipRequires(tok->next());
            if (end)
    ...
    for (; passCount < passCountMax; ++passCount) {
        ...
        if (mSettings->debugtemplate && mSettings->debugnormal) {
            const std::string title("Template Simplifier pass " + std::to_string(passCount + 1));
