...
namespace clangimport {
    struct Data {
        ...
        void enumDecl(const std::string &addr, Token *nameToken, Enumerator *enumerator) {
            const Decl decl(nameToken, enumerator);
        ...
        void funcDecl(const std::string &addr, Token *nameToken, Function *function) {
            const Decl decl(nameToken, function);
            mDeclMap.insert(std::pair<std::string, Decl>(addr, decl));
...
{
    const std::vector<AstNodePtr> children2{astNode};
...
Token *clangimport::AstNode::createTokens(TokenList *tokenList)
{
    if (nodeType == ArraySubscriptExpr) {
        ...
        Token * bracket1 = addtoken(tokenList, "["); // FN
        Token * const index = children[1]->createTokens(tokenList);
        Token * bracket2 = addtoken(tokenList, "]"); // FN
    ...
    if (nodeType == BinaryOperator) {
        ...
        Token *binop = addtoken(tokenList, unquote(mExtTokens.back())); // FN
    ...
    if (nodeType == CaseStmt) {
        Token *caseToken = addtoken(tokenList, "case"); // FN
    ...
    if (nodeType == ConditionalOperator) {
        ...
        Token *tok1 = addtoken(tokenList, "?"); // FN
        Token * const expr2 = children[1]->createTokens(tokenList);
        Token *tok2 = addtoken(tokenList, ":"); // FN
    ...
    if (nodeType == CompoundAssignOperator) {
        ...
        Token *assign = addtoken(tokenList, getSpelling()); // FN
    ...
    if (nodeType == CXXConstructExpr) {
        ...
        Token *par1 = addtoken(tokenList, "("); // FN
        Token *par2 = addtoken(tokenList, ")"); // FN
        par1->link(par2);
        par2->link(par1);
    ...
    if (nodeType == CXXForRangeStmt) {
        ...
        Token * colon = addtoken(tokenList, ":"); // FN
        AstNodePtr range;
        for (int i = 0; i < 2; i++) {
    ...
    if (nodeType == InitListExpr) {
        ...
        Token *start = addtoken(tokenList, "{"); // FN
        start->scope(scope);
        for (const AstNodePtr& child: children) {
        ...
        Token *end = addtoken(tokenList, "}"); // FN
        end->scope(scope);
        start->link(end);
        end->link(start);
    ...
    if (nodeType == MemberExpr) {
        ...
        Token *dot = addtoken(tokenList, "."); // FN
...
void clangimport::AstNode::createTokensFunctionDecl(TokenList *tokenList)
{
    ...
    for (int i = 0; i < children.size(); ++i) {
        const AstNodePtr child = children[i];
...
static void setValues(Tokenizer *tokenizer, SymbolDatabase *symbolDatabase)
{
    ...
    for (Token *tok = const_cast<Token*>(tokenizer->tokens()); tok; tok = tok->next()) {
        if (Token::simpleMatch(tok, "sizeof (")) {
            const ValueType vt = ValueType::parseDecl(tok->tokAt(2), settings);
...
void clangimport::parseClangAstDump(Tokenizer *tokenizer, std::istream &f)
{
    ...
    while (std::getline(f,line)) {
        ...
        const AstNodePtr newNode = std::make_shared<AstNode>(nodeType, ext, &data);
